...
function R(t, i, e, s) {
  t.style.transition = i ? `${i} ${e}ms ${s || U}` : 'none';
}
function L(t, i, e) {
...
const f = { IDLE: 'idle', LOADING: 'loading', LOADED: 'loaded', ERROR: 'error' };
function K(t) {
  return ('button' in t && 1 === t.button) || t.ctrlKey || t.metaKey || t.altKey || t.shiftKey;
}
function X(t, i, e = document) {
  let s = [];
...
class j {
  ...
  calculateSize() {
    const { pswp: t } = this;
    u(this.panAreaSize, N(t.options, t.viewportSize, this.data, this.index)),
...
class it {
  ...
  change() {
    ...
    if ((this.pswp.animations.stopAll(), i.isShifted())) {
      ...
      (t.x < -O && e < 0) || (t.x < 0.1 && e < -0.5)
        ? ((s = 1), (t.x = Math.min(t.x, 0)))
        : ((t.x > O && e > 0) || (t.x > -0.1 && e > 0.5)) && ((s = -1), (t.x = Math.max(t.x, 0))),
...
class nt {
  ...
  start() {
    ...
    t && ((this._startZoomLevel = t.currZoomLevel), u(this._startPan, t.pan)),
  ...
  correctZoomPan(t) {
    ...
    t &&
      ...
      e.setZoomLevel(s);
    const p = !x(l, r);
    if (!p && !o && !h) return e._setResolution(n), void e.applyCurrentZoomPan();
    i.animations.stopAllPan(),
      i.animations.startSpring({
        ...
        onUpdate: (t) => {
          if (((t /= 1e3), p || o)) {
            if ((p && ((e.pan.x = r.x + (l.x - r.x) * t), (e.pan.y = r.y + (l.y - r.y) * t)), o)) {
              const i = s + (n - s) * t;
              e.setZoomLevel(i);
            }
...
class ot {
  ...
  _doClickOrTapAction(t, i, e) {
    ...
    if (!s.dispatch(o, { point: i, originalEvent: e }).defaultPrevented) {
      if ('function' == typeof a) return void a.call(s, i, e);
      switch (a) {
        case 'close':
        case 'next':
          s[a]();
        ...
        case 'zoom':
          ...
          break;
        case 'zoom-or-close':
          n?.isZoomable() && n.zoomLevels.secondary !== n.zoomLevels.initial
            ? n.toggleZoom(i)
  ...
  onPointerDown(t) {
    ...
    e.opener.isOpen
      ? e.dispatch('pointerDown', { originalEvent: t }).defaultPrevented ||
        ...
        1 === this._numActivePoints && ((this.dragAxis = null), u(this.startP1, this.p1)),
  ...
  _updatePoints(t, i) {
    if (this._pointerEventEnabled) {
      ...
      'up' === i && s > -1
        ...
        this._numActivePoints > 0 && u(this.p1, this._ongoingPointers[0]),
        this._numActivePoints > 1 && u(this.p2, this._ongoingPointers[1]);
    } else {
      const e = t;
      (this._numActivePoints = 0),
  ...
  resize(t) {
    ...
    s && ((this.slideWidth = e), this.moveTo(this.getCurrSlideX())),
      this.itemHolders.forEach((i, e) => {
        s && v(i.el, (e + this._containerShiftIndex) * this.slideWidth), t && i.slide && i.slide.resize();
      });
  }
  resetPosition() {
    (this._currPositionIndex = 0),
      ...
      i > 0
        ? ((e = this.itemHolders.shift()),
          e &&
            ...
            v(e.el, (this._containerShiftIndex + 2) * this.slideWidth),
            t.setContent(e, t.currIndex - s + n + 2)))
        : ((e = this.itemHolders.pop()),
          e &&
            (this.itemHolders.unshift(e),
            this._containerShiftIndex--,
            v(e.el, this._containerShiftIndex * this.slideWidth),
            t.setContent(e, t.currIndex + s - n - 2)));
...
class ut {
  ...
  _onKeyDown(t) {
    const { pswp: i } = this;
    if (i.dispatch('keydown', { originalEvent: t }).defaultPrevented || K(t)) return;
    let e,
      s,
      n = !1;
    const o = 'key' in t;
...
class ft {
  constructor(t) {
    this.props = t;
    const { target: i, onComplete: e, transform: s, onFinish: n = () => {}, duration: o = 333, easing: a = mt } = t;
    this.onFinish = n;
    const h = s ? 'transform' : 'opacity',
      r = t[h] ?? '';
    (this._target = i),
      (this._onComplete = e),
      (this._finished = !1),
...
class xt {
  constructor(t, i) {
    ...
    let s = i.html;
    if (!1 === t.options[e]) return;
    'string' == typeof t.options[e + 'SVG'] && (s = t.options[e + 'SVG']), t.dispatch('uiElementCreate', { data: i });
    let n = '';
    i.isButton
      ? ((n += 'pswp__button '), (n += i.className || `pswp__button--${i.name}`))
      : (n += i.className || `pswp__${i.name}`);
    let o = i.isButton ? i.tagName || 'button' : i.tagName || 'div';
    o = o.toLowerCase();
    const a = m(n, o);
    if (i.isButton) {
      'button' === o && (a.type = 'button');
      let { title: s } = i;
      const { ariaLabel: n } = i;
      'string' == typeof t.options[e + 'Title'] && (s = t.options[e + 'Title']), s && (a.title = s);
      const h = n || s;
      h && a.setAttribute('aria-label', h);
    }
    (a.innerHTML = St(s)),
      i.onInit && i.onInit(a, t),
      i.onClick &&
        (a.onclick = (e) => {
          'string' == typeof i.onClick ? t[i.onClick]() : 'function' == typeof i.onClick && i.onClick(e, a, t);
        });
    const h = i.appendTo || 'bar';
    let r = t.element;
    'bar' === h
      ? (t.topBar || (t.topBar = m('pswp__top-bar pswp__hide-on-close', 'div', t.scrollWrap)), (r = t.topBar))
      : (a.classList.add('pswp__hide-on-close'), 'wrapper' === h && (r = t.scrollWrap)),
      r?.appendChild(t.applyFilters('uiElement', a, i));
  }
}
function H(t, i, e) {
  ...
  Ct = {
    ...
    onInit: (t, i) => {
      ...
      const n = (i) => {
        ...
        o = () => {
          if (!i.currSlide?.content.isLoading()) return n(!1), void (s && (clearTimeout(s), (s = null)));
          s ||
            (s = setTimeout(() => {
...
function Zt(t, i, e) {
  ...
  return (
    ...
    a && (o = i.thumbCropped ? Et(a, i.width || i.w || 0, i.height || i.h || 0) : Ot(a)),
...
class Mt {
  ...
  addFilter(t, i, e = 100) {
    this._filters[t] || (this._filters[t] = []),
      ...
      this._filters[t]?.sort((t, i) => t.priority - i.priority),
  ...
  on(t, i) {
    this._listeners[t] || (this._listeners[t] = []), this._listeners[t]?.push(i), this.pswp?.on(t, i);
  }
  off(t, i) {
    this._listeners[t] && (this._listeners[t] = this._listeners[t].filter((t) => i !== t)), this.pswp?.off(t, i);
  }
  dispatch(t, i) {
...
class Ft {
  ...
  load(t, i) {
    ...
    (this.element && !i) ||
      ...
      (this.isImageContent()
        ? ((this.element = m('pswp__img', 'img')), this.displayedImageWidth && this.loadImage(t))
        : ((this.element = m('pswp__content', 'div')), (this.element.innerHTML = this.data.html || '')),
  ...
  onLoaded() {
    (this.state = f.LOADED),
      this.slide &&
        ...
        (this.state === f.LOADED || this.state === f.ERROR) && this.removePlaceholder());
  }
  onError() {
    (this.state = f.ERROR),
      this.slide &&
  ...
  isLoading() {
    return this.instance.applyFilters('isContentLoading', this.state === f.LOADING, this);
  }
  isError() {
  ...
  isZoomable() {
    return this.instance.applyFilters('isContentZoomable', this.isImageContent() && this.state !== f.ERROR, this);
  }
  updateSrcsetSizes() {
  ...
  displayError() {
    if (this.slide) {
      ...
      (t.innerText = this.instance.options?.errorMsg ?? ''),
        ...
        (this.element = m('pswp__content pswp__error-msg-container', 'div')),
  ...
  append() {
    ...
    if (((this.isAttached = !0), this.state === f.ERROR)) return void this.displayError();
...
function W(t, i, e) {
  ...
  if (o) {
    let a;
    (n = new k(o, t, -1)), (a = i.pswp ? i.pswp.viewportSize : B(o, i));
    const h = N(o, a, t, e);
...
class Ht extends Mt {
  getNumItems() {
    ...
    const e = this.dispatch('numItems', { dataSource: i, numItems: t });
  ...
  getItemData(t) {
    ...
    Array.isArray(i)
      ? (e = i[t])
      : i && 'gallery' in i && (i.items || (i.items = this._getGalleryDOMElements(i.gallery)), (e = i.items[t]));
    let s = e;
  ...
  _getGalleryDOMElements(t) {
    return this.options?.children || this.options?.childSelector
      ? X(this.options.children, this.options.childSelector, t) || []
      : [t];
  }
  _domElementToItemData(t) {
    ...
    if (e) {
      ...
      s && ((i.msrc = s.currentSrc || s.src), (i.alt = s.getAttribute('alt') ?? '')),
  ...
  _setClosedStateZoomPan(t) {
    ...
    if (this._croppedZoom && e && this._cropContainer1 && this._cropContainer2) {
      ...
      t
        ...
        : (v(this._cropContainer1, i, s), v(this._cropContainer2, o, a));
    }
    s &&
...
class Ut extends Ht {
    ...
    this.contentLoader.removeByIndex(t),
      this.mainScroll.itemHolders.forEach((i, e) => {
        ...
        this.canLoop() && (s = this.getLoopedIndex(s)),
          s === t && (this.setContent(i, t, !0), 1 === e && ((this.currSlide = i.slide), i.slide?.setIsActive(!0)));
      }),
      this.dispatch('change');
  }
  setContent(t, i, e) {
  ...
  _createMainStructure() {
    (this.element = m('pswp', 'div')),
      ...
      (this.scrollWrap = m('pswp__scroll-wrap', 'section', this.element)),
